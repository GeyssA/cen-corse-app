...
function PhotoCarousel() {
  ...
  const handleTouchStart = (_e: React.TouchEvent) => {
    setTouchStart(_e.targetTouches[0].clientX)
  }

  const handleTouchMove = (_e: React.TouchEvent) => {
    setTouchEnd(_e.targetTouches[0].clientX)
  }
...
export default function Presentation() {
  // const { profile } = useAuth() // Temporairement commenté car non utilisé
  const { theme } = useTheme()
  // const [isEditing, setIsEditing] = useState(false) // Temporairement commenté car non utilisé
