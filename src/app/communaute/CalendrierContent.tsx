...
function CommunauteContent() {
  ...
  const [pendingUpdates, setPendingUpdates] = useState<Record<string, { activityUpdates: Partial<Activity> }>>({})
  ...
  useEffect(() => {
    if (editActivity) {
      setEditPollOptions(editActivity.pollOptions?.map((option: { option_text: string }) => option.option_text) || [])
  ...
  const toggleVote = async (pollOptionId: string, activityId: string) => {
    ...
    try {
              ...
              pollOptions: activity.pollOptions.map(option => {
                if (option.id === pollOptionId) {
                  ...
                  if (hasVoted) {
                    ...
                    return {
                      ...option,
                      votes: currentVotes.filter((vote: { voter_id: string }) => vote.voter_id !== profile.id)
                    }
                  } else {
                    // Ajouter le vote
                    return {
      ...
      if (voterNames[pollOptionId]) {
        setVoterNames(prev => {
          const { [pollOptionId]: _unused, ...rest } = prev
          void _unused
          return rest
        })
      }
  ...
  const getVoterNames = async (optionId: string): Promise<string[]> => {
    try {
      ...
      // Récupérer les profils directement depuis Supabase
      // Si RLS bloque, on utilise les IDs comme fallback
      ...
      if (profileError || !profiles) {
        console.warn('Erreur lors de la récupération des profils, utilisation des IDs:', profileError)
      ...
      const voterNames = voterIds.map(id => {
        ...
        return profile?.full_name || profile?.email || `Utilisateur ${id.slice(0, 8)}`
      })
                            ...
                            <div className="space-y-2">
                              ...
                              <div className={`flex items-center space-x-2 px-4 py-2 ${theme === 'light' ? 'bg-white/90 border-blue-200/60' : 'bg-gradient-to-r from-slate-800/30 to-slate-700/20 border-slate-400/20'} rounded-lg border`}>
                                ...
                                <div>
                                  ...
                                  <div className={`text-xs font-semibold ${theme === 'light' ? 'text-gray-800' : 'text-slate-300'}`}>{formatDateToDDMMYYYY(activity.activity_date)}</div>
                                </div>
                              </div>
                            </div>
                        ...
                        {activity.pollOptions && activity.pollOptions.length > 0 ? (
                          <div className={`space-y-2 p-3 rounded-lg ${
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {activity.pollOptions.map((option: { id: string; option_text: string; votes?: { voter_id: string }[] }) => {
