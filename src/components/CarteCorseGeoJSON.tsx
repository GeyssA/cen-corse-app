...
export default function CarteCorseGeoJSON() {
  const { theme } = useTheme()
  ...
  useEffect(() => {
    fetch('/2A_2B_app.geojson')
      .then(response => response.json())
      .then(data => setGeoData(data))
      .catch(error => console.error("Error loading GeoJSON:", error))
  }, [])

  // Fonction pour convertir les coordonnées GeoJSON en coordonnées SVG
  const convertCoordinates = (coordinates: number[][][]) => {
    ...
    return coordinates.map(ring => 
      ring.map(coord => 
        `${(coord[0] - minX) * scale + offsetX},${(coord[1] - minY) * scale + offsetY}`
      ).join(' ')
    ).join(' ')
  }
        ...
        <div className="relative w-full h-full max-w-4xl">
          ...
          >
            {geoData.features.map((feature: any, index: number) => {
              ...
              const pathData = convertCoordinates(coordinates)
              const zoneId = index === 0 ? 'sud' : 'pays-bastiais' // 2A = Sud, 2B = Nord
              
              return (
                <path
                  key={feature.properties.code_dept}
                  d={`M ${pathData} Z`}
                  fill={index === 0 ? '#f2b936' : '#ec8258'}
                  stroke="white"
                  strokeWidth="3"
