...
export default function ForgotPasswordForm({ onBackToLogin }: ForgotPasswordFormProps) {
  ...
  let authContext: { resetPassword?: (email: string) => Promise<void> } | null = null
  try {
    authContext = useAuth()
  } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si result.error existe et contient une erreur
      if (result && result.error) {
        setError(result.error.message || 'Erreur inconnue')
      } else {
        // Sinon, on considère que c'est un succès (même si result est undefined ou result.error est null)
        console.log('[ForgotPasswordForm] Succès - pas d\'erreur détectée')
        setSuccess(true)
      }
    } catch (err: unknown) {
      finished = true
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Vérifiez votre boîte de réception (et les spams) puis cliquez sur le lien pour définir un nouveau mot de passe.
            </p>
            <button
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              {debug && (
                <pre className="text-xs text-gray-500 mt-2 bg-gray-100 p-2 rounded overflow-x-auto">{JSON.stringify(debug, null, 2)}</pre>
              )}
            </div>
          )}
