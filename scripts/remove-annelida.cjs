/**
 * Supprime les lignes dont le phylum est Annelida dans taxon-filtered.txt.
 * Usage: node scripts/remove-annelida.cjs
 */
...
async function run() {
  ...
  for await (const line of rl) {
    ...
    if ((kept + removed) % 100000 === 0) {
      console.log('Traité:', kept + removed, '| Gardées:', kept, '| Annelida supprimées:', removed);
    }
  }
  ...
  console.log('Terminé. Gardées:', kept, '| Annelida supprimées:', removed);
